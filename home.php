...
<?php 
if($user_name == 'charity'){
  
  if (isset($_POST['send'])){
$body = $_POST['body'];
$date_added = date('d-m-y');

$insert  =  mysql_query("INSERT into club_ VALUES('','$user_name','$date_addeed','$body')");
  }
...
$run = mysql_query("INSERT into meeting VALUES('','$date_added','$user_name','','$place','$date','$time','$reason','no')");
echo '<div class="alert alert-dismissible alert-success">
  ...
  echo '
  <h2 align="center">Service Learning Program</h2>
   <form method="POST" action="home.php">
<h3>Create an invitaion letter for your club below:</h3>
<div class="jumbotron"><div class="jumbotron">
<div class="form-group">
      <label for="textArea" class="col-lg-2 control-label">Body</label>
      <div class="col-lg-10">
        <textarea name="body" class="form-control" rows="3" id="textArea"></textarea>
        <span class="help-block">Please fill out the above field.</span>
      </div>
    </div>
<button name="send" class="btn btn-primary btn-lg btn-block">Send</button>
    </div></form>
   ';
echo "
<p/><p/>
...
<textarea class='form-control' name='reason'   rows='4' cols='58' id='textArea'></textarea><p/>
<input type='submit' name='schedule' value='SCHEDULE'    class='btn btn-primary btn-lg btn-block' placeholder='SCHEDULE'></fieldset>
  </form>
</div></div>
...
while($row = mysql_fetch_assoc($getposts)){
    $id = $row['id'];
    $body = $row['body']; 
    $date_addeed = $row['date_added'];
    $date_added =  date("Y-m-d" , $date_addeed);
...
$if_exist = mysql_query("SELECT * from post_reactions where post_id =  '$id'") or die (mysql_error());

  echo'

<div class="btn-group">

  <a href="#" class="btn btn-default">View Reactions</a>
  <a aria-expanded="true" href="#" class="btn btn-default dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></a>
  <ul class="dropdown-menu">     <form action="home.php"  method="post" enctype="multipart/form-data">
    
  '; 
 
 while ($get_reactor = mysql_fetch_assoc($if_exist)){
  $reactions = $get_reactor['type'];
  $reactor = $get_reactor['reacted_by'];
echo '
<li>'.$reactor.'<button class="'.$reactions.'2" ></button></li>
';}
  echo'
...
<tr><td></td><td>
";
$grab_letters = mysql_query("SELECT * FROM club_ where added_by='charity'");
$numrows = mysql_numrows($grab_letters);
if($numrows != 0) {
  echo '
<h3>Received Invitation Letters</h3>';
while($get_letters = mysql_fetch_assoc($grab_letters)){
  // declare variable for the meeting exctracted from the database
      $date_added = $get_letters['date_added'];
$added_by = $get_letters['added_by'];
$body = $get_letters['body'];

echo '

<div class="panel panel-default">
  <div class="panel-body">
  <h4>From: '.$added_by.' Club</h4>
   <p>'.$body.'</p>
'.$date_added.'
  </div>
</div>
...
$if_exist = mysql_query("SELECT * from post_reactions where reacted_to = '$added_by' AND post_id =  '$id'") or die (mysql_error());
$count = mysql_num_rows($if_exist);
...
<div class="btn-group">
  ...
  <a href="#" class="btn btn-default">React</a>
  <a aria-expanded="true" href="#" class="btn btn-default dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></a>
  <ul class="dropdown-menu">     <form action="home.php"  method="post" enctype="multipart/form-data">
    <li><button name="like'.$id.'" class="likee2" ></button></li>
    <li><button name="amazed'.$id.'" class="amazed2" ></button></li>
    <li><button name="tired'.$id.'" class="tired2" ></button></li></form>
...
';}
  ...
  $get_reactions = mysql_query("SELECT * from post_reactions where reacted_to = '$added_by' AND post_id =  '$id'");
  while($get = mysql_fetch_assoc($get_reactions)){
    $class = $get['type'];
...
$like = 'likee';
 ...
 $like_query = mysql_query("INSERT INTO post_reactions VALUES ('','$user_name','$added_by','likee','$id')");

}

if (isset($_POST['amazed'.$id])){
$amazed = 'amazed';
 
 $amazed_query = mysql_query("INSERT INTO post_reactions VALUES ('','$user_name','$added_by','amazed','$id')");
}


if (isset($_POST['tired'.$id])){
$tired = 'tired';
 
 $tired_query = mysql_query("INSERT INTO post_reactions VALUES ('','$user_name','$added_by','tired','$id')");
